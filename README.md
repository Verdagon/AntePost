...
- `rust-twomuts/` — minimal demo of Rust's compile-time rule against two simultaneous `&mut` to the same data.
- `swift-twomuts/` — minimal demo of Swift's compile-time rule against two `inout` arguments aliasing.
...
cargo run --manifest-path rust-twomuts/Cargo.toml

swift run --package-path swift-spaceship
swift run --package-path swift-union
swift run --package-path swift-twomuts
```
